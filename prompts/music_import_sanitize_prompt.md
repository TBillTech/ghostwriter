...
Sanitized Melody Import (measure,pitch,duration)
------------------------------------------------
[SANITIZED_IMPORT]
...
2. Quantize durations to eighth-beat increments (for example, for 4/4 time eighth note is 0.5) while keeping measures as whole numbers.
...
Output
------
Return exactly one CSV block titled `CORE_MELODY.csv` with the header `measure,pitch,duration` followed by the cleaned rows. Do not include any other prose before or after the CSV block.
