...
- [ ] When an import exists for the melody voice, skip `music_melody_emotion_prompt.md` and `music_emotion_chord_prompt.md`. Instead, run a new `music_import_sanitize_prompt.md` that consumes the sanitized `score.musiccsv` (or freshly generated `import.musiccsv`), quantizes timestamps, trims leading/trailing empty measures, and emits CORE-ready rows (`measure,pitch,duration`). The pipeline will enforce `semi_tones=(0)` and derives the `transition` column directly from the provided pitch values so the dwell/root logic follows the imported contour.
