"""Mixed pipeline (dialog + brief narration/action)."""
...
def build_mixed_batch_user(
    *,
    ...
    """Construct the exact USER content for the mixed batch call.

    Mirrors the runtime composition logic so other components (like mock regeneration)
    can reuse it for byte-for-byte parity.
    """
    import re as _re
...
def run_mixed_pipeline(tp, state, *, ctx: RunContext, tp_index: int, prior_paragraph: str = "", log_dir: Optional[Path] = None) -> str:
    ...
    if existing_bs and brainstorm_has_done(existing_bs):
        ...
        brainstorm_new = llm_call_with_validation(
            ...
            log_maker=None,
            context_tag=f"tp={tp_index:02d} type=mixed template={tpl1} step=BRAIN_STORM",
        )
    ...
    disable_order_mixed = (os.getenv("GW_DISABLE_ORDERING_MIXED", "0") == "1") or (os.getenv("GW_DISABLE_ORDERING_IMPLICIT", "0") == "1")
    if disable_order_global or disable_order_mixed:
        ordered = brainstorm
    else:
        sys2 = "Order mixed beats for flow."
        ...
        ordered = llm_call_with_validation(
            ...
            context_tag=f"tp={tp_index:02d} type=mixed template={tpl2} step=ORDERING",
        )

    # Actor assignment
    reps3 = dict(reps2); reps3["[ORDERED_BULLETS]"] = ordered
    sys3 = "Assign lines to actors as 'id: line' entries (Narrator allowed for brief prose beats)."
    ...
    actor_lines = llm_call_with_validation(
        ...
        context_tag=f"tp={tp_index:02d} type=mixed template={tpl3} step=ACTOR_ASSIGNMENT",
    )
    ...
    # Body language, Agenda, Reactions (reuse dialog prompts)
    ...
    body_lang = llm_call_with_validation(
        ...
        context_tag=f"tp={tp_index:02d} type=mixed template={tpl4a} step=BODY_LANGUAGE",
    )
    # Agenda
    ...
    agenda_text = llm_call_with_validation(
        ...
        context_tag=f"tp={tp_index:02d} type=mixed template={tpl4b} step=AGENDA",
    )
    ...
    reactions_text = llm_call_with_validation(
        ...
        context_tag=f"tp={tp_index:02d} type=mixed template={tpl4c} step=REACTIONS",
    )
    ...
    # Render lines (Narrator allowed) using batch character-dialog composition reused from dialog pipeline
    outputs: List[str] = []
    lines_pairs: List[Tuple[str, str]] = actor_pairs
    ...
    # Build batch USER with shared helper for exact parity
    batch_user = build_mixed_batch_user(
        reps4=reps4,
    ...
    batch_resp = llm_call_with_validation(
        ...
        context_tag=f"tp={tp_index:02d} type=mixed template=character_dialog_prompt.md step=CHARACTER_DIALOG_BATCH",
    )

    # Parse and post-process
    _A2 = _re.compile(r"^\s*([A-Za-z0-9_.\-]+)\s*:\s+(.+)")
